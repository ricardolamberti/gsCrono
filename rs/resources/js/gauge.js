...
//    	   size = Math.max(width, height);
       ...
       nv.addGraph(function() {
         ...
         .x(function(d) { return d.key.length>10?d.key.substring(0,10)+"...":d.key })
         .y(function(d) { return d.y })
         .color(d3.scale.myColors().range())
         .labelThreshold(.03)  // Configure the minimum slice size for labels
								// to show up
         .labelType("percent")
         .donut(true).donutRatio(0); /*
										 * Trick to make the labels go inside
										 * the chart
										 */
...
//	    	   linker = $(this)[0].__data__.data.link;
//	    	   if (linker!="") {
//	    		   goRefresh();
//	    	   };
//	    	})
...
//    var size = Math.max(width, height);
	...
	nv.addGraph({
		 generate : function() {
			...
			.stacked(isStacked).showControls(showControls).showLegend(showLegend).reduceXTicks(reduce).color(myColors);
...
//			svg.append("svg:image").attr('x', 0).attr('y', 0)
//					.attr('width', 300).attr('height', 200).attr('opacity', .2)
