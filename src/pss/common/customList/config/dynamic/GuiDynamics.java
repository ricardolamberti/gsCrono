...
//  		c.setDisplayOnly(true);
  	...
  	while (iterE.hasMoreElements()) {
  		...
  		try {
			...
			if (c.getCalculeDiferencia()) {
	  		try {
	    		 l=zLista.AddColumnaLista(JFloat.class, "Dif."+c.getNombreColumna(), c.getDifProperty(c.getNameFieldDif(),"Dif."+c.getNombreColumna()));
	  		} catch (Exception e) {
	  		...
	  		if (c.getPorcentajeDiferencia()){
		  		try {
		    		 l=zLista.AddColumnaLista(JFloat.class, "Dif.Porc."+c.getNombreColumna(), c.getDifProperty(c.getNameFieldDifPorc(),"Dif.porc."+c.getNombreColumna()));
		  		} catch (Exception e) {
		  			PssLogger.logError(e);
		  			l=zLista.AddColumnaLista(JString.class, "Error", null);
		  		}
			...
			if (ranking!=null) {
	  		JColumnaLista lo=null;
				lo = zLista.AddColumnaLista(new JLong().getObjectClass(), ranking.getNameField(), ranking.getRankProperty());
				lo.setVisible(true);
				lo.setMatrix(false);
			}
  	}
  ...
  public void ConfigurarMatrix(JWinMatrix zMatrix) throws Exception {
  	...
  	while (iter.hasMoreElements()) {
  		...
  		BizCampo ranking = c.getObjCampoOrden();
  	  JEjeMatrix eje = zMatrix.AddEjeMatrix(c.getForceNombreColumna(),c.isRolFila()?JEjeMatrix.FILA:JEjeMatrix.COLUMNA, p);
  	  eje.setEje(c);
  		...
  		if (rel==null) continue;
			...
			if (f.getCalculeDiferencia()) {
				...
				JString objDescrDif = new JString();
	 			...
	 			source.loadFieldFromDataSet(f.getNameFieldDif(), objDif, set);
 				source.loadFieldFromDataSet("xdiffcol_"+f.getNameField(), objDescrDif, set);

 				w.getRecord().addItem(f.getNameFieldDif(), objDif);
				w.getRecord().addItem("xdiffcol_"+f.getNameField(), objDescrDif);
	 			
				if (f.getPorcentajeDiferencia()) {
	 				source.loadFieldFromDataSet(f.getNameFieldDifPorc(), objPorcDif, set);
					w.getRecord().addItem(f.getNameFieldDifPorc(), objPorcDif);
	 			}
	 		}
			if (p!=null && !p.isTableBased()) {
  			boolean read=false;
