...
public class FormCustomListResultMatriz extends JBaseForm {
    ...
    if (getWin().showFilter()) {
			...
			row1.AddItemMatrix(100, "Informe").setMode(JFormLista.MODE_CUSTOMLIST).setShowFooterBar(false).setForceHidePaginationBar(true);
		} else {
			JFormPanelResponsive row1 = AddItemColumn(12).setZoomtofit(1500);
			row1.AddItemMatrix(100).setMode(JFormLista.MODE_CUSTOMLIST).setShowFooterBar(false).setAllowSortedColumns(false).setForceHidePaginationBar(true);
