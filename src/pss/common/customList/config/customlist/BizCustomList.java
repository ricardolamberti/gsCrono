...
//  private JLong pWidth = new JLong();
  ...
  private JBoolean pViewTable = new JBoolean();
...
//  			continue;
  	...
  	if (extraFiltros!=null) {// estos filtros vienen del informe padre
    	...
    	while (itEF.hasMoreElements()) {
    		...
    		if (!c.isFiltroActivo()) continue;
    		if (!c.getRecordSource().equals(this.getObjRelation().getClassTarget()))
    			continue;
   			filtros.getStaticItems().addElement(c);
    	}
  	}
...
//		JMap<Long,Long> equivCampos = JCollectionFactory.createMap();
		...
		while (it.hasMoreElements()) {
			...
			na.insert();
//			equivCampos.addElement(ampl.getSecuencia(), na.getIdentity("secuencia"));
		}
		...
		while (iter.hasMoreElements()) {
			...
			if (r.isRelationRaiz()) {
				...
				while (iter2.hasMoreElements()) {
					...
					c2.setDescrCompleta(r.getBeautyDescrCampo()+", "+c2.getDescripcion());
  ...
  public BizSqlEventHistory  generarAviso(JFilterMap a,BizSqlEventAction action,boolean vistaPrevia) throws Exception {
	...
	public String getCorreoAviso(JFilterMap a, BizSqlEventAction action,BizSqlEventHistory hist) throws Exception {
		...
		if (isInforme()) {
			...
			l.setRecord(this);
			l.GetcDato().fillFilters(a);
			return URLEncoder.encode("<html><body><img src=\""+JTools.convertHtml2JPG(new ByteArrayInputStream(l.getHtmlView(26,"htmlfull",a,true,false,false,false).getBytes()),1000,4000)+"\"></body></html>","ISO-8859-1").replace("+", "%20");
		...
		if (action.isOutPantalla()) {
			...
			l.setRecord(this);
			l.GetcDato().fillFilters(a);
			return URLEncoder.encode(l.getHtmlView(24,"html",a),"ISO-8859-1").replace("+", "%20");
		}
		...
		return getObjPlantilla().generateDoc(this.getCompany(),this,a);
  }
