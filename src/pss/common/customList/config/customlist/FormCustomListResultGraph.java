...
public class FormCustomListResultGraph extends JBaseForm {
	 ...
	 */
	public FormCustomListResultGraph() throws Exception {
		try {
			jbInit();
		} catch (Exception e) {
			e.printStackTrace();
    ...
    if (getWin().showFilter()) {
			...
			row2.AddItemImage(null, "image").setForceImage(getWin().getMode()==GuiCustomList.EMBEDDED).setHeight(600);
			if (getWin().GetcDato().isViewTable())
				row2.AddItemMatrix(100).setForceHidePaginationBar(true);
		} else if (getWin().showZoom()) {
			JFormPanelResponsive row1 = AddItemColumn(12).setZoomtofit(1500);
			row1.AddItemImage(null, "image").setForceImage(getWin().getMode()==GuiCustomList.EMBEDDED).setHeight(600);
			if (getWin().GetcDato().isViewTable())
				row1.AddItemMatrix(100).setAllowSortedColumns(false).setShowFooterBar(false).setForceHidePaginationBar(true);

		} else {
			JFormPanelResponsive row1 = AddItemColumn(12).setZoomtofit(1500);
			row1.AddItemImage(null, "image").setForceImage(getWin().getMode()==GuiCustomList.EMBEDDED).setHeight(600);
			if (getWin().GetcDato().isViewTable())
				AddItemMatrix(100).setAllowSortedColumns(false).setShowFooterBar(false).setForceHidePaginationBar(true);
