...
public class FormCampoFilter  extends JBaseForm {
...
//    AddItemEdit( null, CHAR, OPT, "has_filter").setHide(true);
    ...
    AddItemEdit( null, CHAR, OPT, "campo_key2").setHide(true);
    
  	JFormRadioResponsive form = AddItemRadio("Tipo: "+getWin().GetcDato().getRolDescription(), CHAR, OPT, "orientacion", getOptions());
  	form.setSizeColumns(12).SetValorDefault("N").setRefreshForm(true);
  	form.setVisible(!getWin().GetVision().equals("FILTROS"));
	...
	public void checkControls(String sControlId) throws Exception {
		...
		if (fx != null) {
			if (fx.hasOnlyFunction()) {
				...
				getControles().findControl("orientacion").setVisible(!getWin().GetVision().equals("FILTROS"));
			}
		}
  	...
  	if (this.isCampoValue(2, sControlId))
		...
		String s = getWin().GetccDato().getObjectTypeInput();
  	...
  	if (s!=null) {
    	...
    	} else if (s.equals(JObject.JFLOAT) || (s.equals(JObject.JCURRENCY)) ) {
    		if (f.equals("in")) {
    			getControles().findControl("value_edit").setPlaceHolder("Lista de valores separados con ,");
    			getControles().findControl("value_edit2").setPlaceHolder("Lista de valores separados con ,");
    		}
