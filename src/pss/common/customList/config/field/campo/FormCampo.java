...
//    panelOrd.AddItemRadio("Sentido", CHAR, OPT, "orden_ascdesc", getWin().GetcDato().getOptionsOrden()).setAlign(JFormCheckResponsive.LABEL_TOP).setSizeColumns(12).SetValorDefault(BizCampo.ORDER_NO);
    panelOrd.AddItemRadio("Sentido", CHAR, OPT, "orden_ascdesc", getWin().GetcDato().getOptionsOrden()).setSizeColumns(12).SetValorDefault(BizCampo.ORDER_NO);
    ...
    panel.AddItemCombo( "Campo", LONG, OPT, "campo_diferencia", new JControlCombo() {
			public JWins getRecords(boolean one) throws Exception {
				return getCamposCustomList(one);
			}
		}).setSizeColumns(8);
...
//    JFormPanelResponsive panelDim = AddItemFieldset("Filtros Dinámico");
//    panelDim.AddItemCheck( "Filtro Dinámico", CHAR, REQ, "dinamico", "S", "N" );
   }
