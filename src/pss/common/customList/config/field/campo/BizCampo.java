...
//	private JLong pMaxSize = new JLong();
  ...
  private JString pDescrFalse = new JString();
	...
	private JString pCampoDiferencia = new JString();
  ...
  public void setValor2(Date zValue) throws Exception {    
	...
	public void setCampoDiferencia(String zValue) throws Exception {
	 ...
	 */
	public String getCampoDiferencia() throws Exception {
...
//  }
  ...
  public void prepareDiferencia(JBaseRecord r) throws Exception {
		BizField campo = getObjCampoDiferencia();
		...
		if (hasOperador()) {
			r.addField( this.findFunction(true)+" as xdiffcol_"+ getNameField()).setTabla(null);
			r.addField( "("+campo.findFunction(true)+")-lag("+campo.findFunction(true)+") over("+filas+" ORDER BY ("+findFunction(true)+")  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as "+ getNameFieldDif()).setTabla(null);
			if (getPorcentajeDiferencia())
				r.addField( "case when (lag("+campo.findFunction(true)+") over("+filas+" ORDER BY ("+findFunction(true)+")  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING))=0 then 0 else ((("+campo.findFunction(true)+")-(lag("+campo.findFunction(true)+") over("+filas+" ORDER BY ("+this.findFunction(true)+")  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)))*100)/(lag("+campo.findFunction(true)+") over("+filas+" ORDER BY ("+this.findFunction(true)+")  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) end as "+ getNameFieldDifPorc()).setTabla(null);
		} else {
			r.addField( this.getField()+" as xdiffcol_"+ getNameField()).setTabla(null);
			r.addField( campo.getField()+"-lag("+campo.getField()+") over("+filas+" ORDER BY ("+this.getField()+")  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as "+ getNameFieldDif()).setTabla(null);
			if (getPorcentajeDiferencia())
				r.addField( "case when (lag("+campo.getField()+") over("+filas+" ORDER BY ("+this.getField()+")  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING))=0 then 0 else (("+campo.getField()+"-(lag("+campo.getField()+") over("+filas+" ORDER BY ("+this.getField()+")  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)))*100)/(lag("+campo.getField()+") over("+filas+" ORDER BY ("+this.getField()+")  RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)) end as "+ getNameFieldDifPorc()).setTabla(null);
		}

  }
  
  public String getNameFieldDif() throws Exception {
  	return "xdiffval_"+ getNameField();
  	
  }
  public String getNameFieldDifPorc() throws Exception {
  	return "xdiffprc_"+ getNameField();
  	
  }
  
  BizCampo objCampo;
  public BizCampo getObjCampoDiferencia() throws Exception {
  	if (pCampoDiferencia.isNull()) return null;
  	if (objCampo!=null) return objCampo;
  	BizCampo campo = getObjCustomList().findCampo(getCampoDiferencia());
  	return objCampo=campo;
  }
	  ...
	  public void assingFiltersLOV(boolean one, JRecords<?> r, int v) throws Exception {
	  	...
	  	if (rel==null) return;
	    	...
	    	while (itFilter.hasMoreElements()) {
	    		...
	    		String sFilter= JTools.replace((String)filter.firstObject(), "COMPANY_CUSTOMLIST",  getObjCustomList().getCompany());
