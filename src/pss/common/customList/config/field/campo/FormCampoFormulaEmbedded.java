...
public class FormCampoFormulaEmbedded extends JBaseForm {
	...
	public void InicializarPanel(JWin zWin) throws Exception {
		...
		AddItemEdit("Formula", CHAR, OPT, "formato_param").SetReadOnly(true);;

		AddItemCheck("Visible", OPT, "visible").setSizeColumns(2).setRefreshForm(true);
		AddItemCombo("Función", CHAR, OPT, "funcion", new JControlCombo() {
			@Override
			public JWins getRecords(boolean one) throws Exception {
				return getFunciones(one);
			}
		}).setSizeColumns(10).setRefreshForm(true);
		

		AddItemRadio("Tipo: "+getWin().GetcDato().getRolDescription(), CHAR, OPT, "orientacion", getOptions()).SetValorDefault("N").setRefreshForm(true);
		AddItemCheck("Agr.filtro", OPT, "has_filter").setRefreshForm(true);
	...
	public void checkControls(String sControlId) throws Exception {
		...
		if (fx != null) {
			if (fx.hasOnlyFunction()) {
				...
				getControles().findControl("has_filter").setVisible(true);
