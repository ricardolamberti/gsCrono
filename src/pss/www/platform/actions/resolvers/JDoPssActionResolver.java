...
//		getSession().getHistoryManager().getLastHistory().isJSModal()
		...
		if (ajaxcontainer==null) return false;
	...
	protected boolean isBackPageOnDelete() {
       ...
       protected boolean hasToSubmit(JAct action) throws Exception {
		...
		if (this.bNoSubmit) return false;
...
//		if (!this.getRequest().getFormData(tableProvider).isNull()) return true;
	...
	public boolean isSubmit() throws Exception {
		if (getRequest().getArgument("issubmit")==null) return false;
		return getRequest().getArgument("issubmit").equals("true");
	}
	public boolean isBackAfterSubmit() throws Exception {
		if (getRequest().getArgument("isbackaftersubmit")==null) return true;
...
//		return this.goOn();
	...
	private boolean isBackAction(JAct action) throws Exception {
		if (action instanceof JActSubmit) return false;
...
//			return;
	...
	protected JAct getSubmit(JBaseWin actionOwner, BizAction action) throws Exception {
		JAct submit;
 	  if (actionOwner!=null) actionOwner.preRefresh();
		if (action!=null) {
	...
	public boolean isReturnWinLov() throws Exception {
		if (this.getRequest().getArgument("dg_action")==null) return false;
		if (this.getRequest().getArgument("dg_action").endsWith("_4,")) 
			return true;
		return false;
	}
	public boolean needLoadData() throws Exception {
		if (this.getRequest().hasMultipleObjectOwnerList()) return false;
//		if (this.getRequest().getPssObjectOwner().indexOf("_anonimus_")!=-1) return false;
	//	if (isReturnWinLov()) return false;
	  if (this.getRequest().getURI().indexOf("do-SwapListRefreshAction")!=-1) return false;
	  if (this.getRequest().getURI().indexOf("do-WinListRefreshAction")!=-1) return false;
  	if (this.getRequest().getURI().indexOf("do-PreviewAreaAction")!=-1) return false;
...
//		return this.getBaseWinFromBundle(bundle,true);
	...
	public void addMultipleActionOwnersDest(JWins baseWin) throws Exception {
		if (this.getRequest().getMultipleObjectOwnerListDest().equals("")) return;
		baseWin.clearItems();
	...
	public void buildWinsFromField(JBaseWin baseWin,String fields) throws Exception {
		...
		while (st.hasMoreTokens()) {
			...
			if (obj==null) continue; // HGK en seleccion multimple vienen rows que son sub-rows y da error, corregir en el .js para que no vengan
...
//				JWebActionData winBundle=this.getRequest().getData("act_owner");
				...
				if (!obj.getClass().isAssignableFrom(wins.GetClassWin())) continue; // lo mismo que arriba, me aseguro que no se meta un row que no va, truchex!!!
				win=(JWin) obj;
			}
...
//			PssLogger.logDebug("|------------------------------------------------> FIND WIN "+id);
			...
			if (loadData) winFactory.loadData(actionOwner,true,getRequest().hasTableProvider()?getRequest().getTableProvider():null,contextAtionOwner);
			return actionOwner;
		}
		if (obj instanceof String) {
...
//			if (bundle==null) return null;
	...
	protected void storeScroller() throws Exception {
		if (this.findFirstProvider()==null) return;
	...
	public void storeOtherProviders(JBaseWin zActionOwner, BizAction zAction) throws Exception {
		...
		while (en.hasNext()) {
			...
			if (("form_"+tableProvider).equals(getRequest().getTableProvider())) continue;
	...
	private JAct getActOtherProvider(JBaseWin zActionOwnerOrig, BizAction zActionOrig,JHistory h,String tableProvider) throws Exception {
		...
		if (action==null) return null; // no hay accion asoc
		storeMeta(action,provider);
		JWebActionData data=this.getRequest().getFormData("form_"+tableProvider);
		if (data.isNull()) return null; // no tiene datos
	  if (!action.hasSubmit()) return null;// no es submit
		JAct submit = action.getObjSubmit(); 
		if (submit==null) return null;// no tiene jact
		if (submit instanceof JActQuery) return null; //es query
		if (submit instanceof JActQueryActive) return null; //es query
...
//		processSwapElement(submit,"form_"+tableProvider);
	...
	public BizAction resolveAction(JBaseWin owner) throws Exception {
		...
		if (action!=null)	action.clearSubmit();
		if (action==null) action = this.createVirtualAction(owner);
		if (action!=null) {
...
//		action.setObjFilterMap(this.createFilterMap().getElement(action.getProviderName()));
	...
	public BizAction findAction(JBaseWin zActionOwner) throws Exception {
		...
		if (this.getRequest().hasPssObjectOwnerDest()) {
			...
			wins.setDropListener(winsDest.getDropListener());// para el caso de que no se seleccione ninguno, como el drop viene en los elegidos (y en este caso no hay ninguno), lo pongo en este para sacarlo de ahi.
...
//		if (a==null) a=zActionOwner.findAction(Integer.parseInt(JTools.clearNonNumbers(idAction)));
  ...
  public boolean calculeIsModal() throws Exception {
	...
	protected String getTransformer() throws Exception {
		...
		if (sRequestedOutput.equalsIgnoreCase("html")) return "resources/stylesheets/html/responsive_page.xsl";
		...
		if (sRequestedOutput.equalsIgnoreCase("json")) return "resources/stylesheets/export/page.json.filter.xsl";
		if (sRequestedOutput.equalsIgnoreCase("mobile")) return "resources/stylesheets/mobile/page.xsl";
		throw new RuntimeException("Invalid transformer: "+sRequestedOutput);
	}

	protected String getSerializer() throws Exception {
		String sRequestedOutput=JWebActionFactory.getSerializerFor(this.getRequest());
		if (sRequestedOutput.equalsIgnoreCase("htmlfull")) return "htmlfull";
		if (sRequestedOutput.equalsIgnoreCase("html")) return "html";
		if (sRequestedOutput.equalsIgnoreCase("excel")) return "excel";
		if (sRequestedOutput.equalsIgnoreCase("graph")) return "graph";
		if (sRequestedOutput.equalsIgnoreCase("csv")) return "csv";
		if (sRequestedOutput.equalsIgnoreCase("report")) return "report";
		if (sRequestedOutput.equalsIgnoreCase("map")) return "map";
		if (sRequestedOutput.equalsIgnoreCase("json")) return "json";
